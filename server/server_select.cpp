/*
I have used the code refernce from
...
int main(int argc , char *argv[])
{
	...
	char buf[BUFFER_LEN];    // buffer for client data
	int nbytes;
			...
			if (FD_ISSET(i, &read_fds)) { // we got one!!
				if (i == listener) {
					...
					if ((nbytes = recv(i, buf, sizeof buf, 0)) <= 0) {
						...
						else input_data.push_back(buf);

						for (unsigned int k=0;k<input_data.size();k++){	
							to_send += maintain_calendar(input_data[k]) + "\n";
						}

						printf("%s\n",buf);
						if(send(i,to_send.c_str(),to_send.size(),0) == -1){
							perror("send\n");
						}
					}
